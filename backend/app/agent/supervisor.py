...
async def supervisor_node(state: MultiAgentState) -> dict[str, Any]:
    ...
    if hard_limit and elapsed >= hard_limit:
        await log_bus.emit(session_id, "log_entry", {
            "level": "warn", "agent_id": "supervisor",
    ...
    if time_budget and elapsed >= time_budget * 0.85:
        ...
        await log_bus.emit(session_id, "budget_warning", {
            "remaining_sec": remaining, "mode": "time_budget"
        })
    ...
    if spec_level == "L4" and current == "reviewer":
        ...
        if review_reports:
            ...
            await log_bus.emit(session_id, "completion_check", {
                "criteria": criteria, "all_met": all_met
            })
            if all_met and passed and confidence >= state["rules"].get("l4", {}).get("completion_confidence", 0.85):
                return {**state, "current_agent": "export"}

    remaining_sec = int(hard_limit - elapsed) if hard_limit else None
    next_step = _find_step(state, next_agent)

    await log_bus.emit(session_id, "supervisor_routing", {
...
def _skip_to_next(state: MultiAgentState, stuck_agent: str | None = None) -> str:
    ...
    for step_id in sequence:
        if step_id == "export":
            continue
        if step_id not in outputs and call_counts.get(step_id, 0) < ANTI_LOOP_LIMIT and step_id != stuck:
            return step_id
    return "export"
...
def _check_l4_criteria(state: MultiAgentState) -> dict[str, bool]:
    ...
    required = required_artifact_keys(pipeline) if pipeline else {
        "product_spec", "architecture_spec", "api_spec", "ui_spec"
    }
